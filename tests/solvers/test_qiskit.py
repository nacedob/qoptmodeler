...
@pytest.fixture(scope="module")
def solver():
    return QAOASolver(solver='qiskit', n_layers=2)


# Test Pennylane QAOA function
def test_qiskit(easy_problem, solver):
...
def test_max_time(hard_problem, solver):
    ...
    max_time = 16
    start = perf_counter()
    _ = solver.solve(J=J, h=h, silent=False, epochs=100, max_time=max_time)
    measured_time = perf_counter() - start

    assert abs(measured_time - max_time) < 2
...
def test_limit_output(hard_problem, solver):
    ...
    solution = solver.solve(J=J, h=h, silent=False, epochs=2, possible_result_ints=possible_ints)
