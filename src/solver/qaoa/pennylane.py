...
# TODO: jaxear
def _create_cost_operator_pennylane(J: np.ndarray, h: np.ndarray, params: qml.typing.TensorLike, n_qubits: int):
    # single-qubit terms
...
def _create_mixer_operator_pennylane(params, n_qubits, transition_matrix_mixer=None):
    if transition_matrix_mixer in [None, 'x']:
        for i in range(n_qubits):
            qml.RX(2 * params, wires=i)
    else:
        assert isinstance(transition_matrix_mixer, (np.ndarray, spmatrix)), f'Invalid mixer: {transition_matrix_mixer}'
        transition_matrix_mixer_ = transition_matrix_mixer.toarray() if isinstance(transition_matrix_mixer, spmatrix) \
...
def _qaoa_circuit_pennylane(params_cost: qnp.ndarray,
    ...
    for layer in range(len(params_cost)):
        # ---------- COST HAMILTONIAN ----------
        _create_cost_operator_pennylane(J, h, params_cost[layer], n_qubits)
        # ---------- MIXER HAMILTONIAN ----------
        _create_mixer_operator_pennylane(params_mixer[layer], n_qubits, transition_matrix_mixer)
    if draw:
        qml.drawer.use_style("pennylane")
        fig, ax = qml.draw_mpl(_qaoa_circuit_pennylane)(params_cost, params_mixer, J, h, n_qubits)
        import matplotlib.pyplot as plt
        plt.show()


def _cost_function_qaoa_pennylane(params: qnp.ndarray,
                                  n_qubits: int,
                                  J: np.ndarray,
                                  h: np.ndarray,
                                  init_state: qml.typing.TensorLike = None,
    ...
    _qaoa_circuit_pennylane(cost_params, mixer_params, J, h, n_qubits,
...
def _get_optimizer(optimizer, optimizer_options):
    if optimizer_options is None:
        optimizer_options = {}

    if optimizer == 'adam':
        opt = qml.AdamOptimizer(stepsize=optimizer_options.get('lr', 0.01),
                                beta1=optimizer_options.get('beta1', 0.9),
                                beta2=optimizer_options.get('beta2', 0.99))
    elif optimizer == 'gd':
        opt = qml.GradientDescentOptimizer(stepsize=optimizer_options.get('lr', 0.01))
    elif optimizer == 'rms':
        opt = qml.RMSPropOptimizer(stepsize=optimizer_options.get('lr', 0.01),
                                   decay=optimizer_options.get('decay', 0.9))
    elif optimizer == 'momentum':
        opt = qml.MomentumOptimizer(stepsize=optimizer_options.get('lr', 0.01),
                                    momentum=optimizer_options.get('momentum', 0.9))
    elif optimizer == 'nesterov':
        opt = qml.NesterovMomentumOptimizer(stepsize=optimizer_options.get('lr', 0.01),
                                            momentum=optimizer_options.get('momentum', 0.9))
    else:
...
def qaoa_pennylane(J: np.ndarray,
                   h: np.ndarray,
                   n_layers: int,
                   solver: str = 'cpu',
    ...
    :param optimizer_options: (dict, optional) Options for the optimizer.
    ...
    """
    if solver != 'cpu':
        raise NotImplementedError("Only CPU solver is implemented for PennyLane")
    ...
    # Init optimization
    n_qubits = J.shape[0]
    qnode = qml.QNode(_cost_function_qaoa_pennylane,
    ...
    opt = _get_optimizer(optimizer, optimizer_options)
    ...
    # Get most probable (with no shots, so prob is exact)
    qnode_fixed_args_ = qnode_fixed_args.copy()
    del qnode_fixed_args_['probs']
    qnode = qml.QNode(_cost_function_qaoa_pennylane, qml.device("default.qubit"))
